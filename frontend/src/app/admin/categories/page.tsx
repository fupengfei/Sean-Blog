...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId !== null) {
        await adminUpdateCategory(editingId, modalName.trim(), modalSlug.trim());
      } else {
        await adminCreateCategory(modalName.trim(), modalSlug.trim());
      }
      setShowModal(false);
      fetchCategories();
    } catch (err: unknown) {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-on-surface-variant font-ui text-sm">暂无分类</p>
        </div>
      ) : (
